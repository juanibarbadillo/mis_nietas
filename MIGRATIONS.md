# Migraciones Supabase · Regionales Mis Nietas
...
-- Reemplazar <TU-WHATSAPP> con el número en formato internacional sin + ni espacios (ej: 5492604xxxxxx)
-- IMPORTANTE: el slug DEBE ser 'misnietas' (es el que usa la web para resolver el negocio).
...
-- Backfill (solo si ya tenías datos sueltos sin negocio_id; en una DB nueva no hace falta)
update productos      set negocio_id = (select id from negocios where slug='misnietas') where negocio_id is null;
update pedidos        set negocio_id = (select id from negocios where slug='misnietas') where negocio_id is null;
update zonas_delivery set negocio_id = (select id from negocios where slug='misnietas') where negocio_id is null;
```
