# Setup integración Google Sheets — Regionales Mis Nietas
...
1. Bajá el archivo [`docs/sheet-template.csv`](./sheet-template.csv) de este repo.
   - Si lo tenés clonado: está en `docs/sheet-template.csv`.
...
7. Renombrá el archivo: arriba a la izquierda donde dice el nombre → poné, ej., `Pedidos Regionales Mis Nietas`.

#### 🅑 Opción "todo desde Apps Script"

1. **Nuevo → Google Sheets en blanco** en Drive.
2. Renombrá el archivo, ej. `Pedidos Regionales Mis Nietas`.
...
7. Cuando termina, te muestra un alert "Sheet configurado". Los headers ya están en la fila 1 con formato Mis Nietas. 🎉
...
3. Configurá:
   - **Descripción**: `Webhook pedidos Regionales Mis Nietas`
...
| **Apps Script Web App URL** | Endpoint HTTP donde el sistema POSTea cada pedido nuevo. | El backend de Regionales Mis Nietas (sin que VOS hagas nada). |
